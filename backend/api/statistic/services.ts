        ...
        if (userLessonItem !== null) {
          switch (userLessonItem.type) {
            ...
            case ELessonType.CodeScript:
              result = {
                completed: (userLessonItem.checkpoint as UserCodescriptLessonCheckpoint).result.filter((item) => item)
                  .length,
                total: (item.resource as CodescriptLessonResourse[]).length,
              };
              break;
            case ELessonType.Selection:
              result = {
                completed: (userLessonItem.checkpoint as UserSelectionLessonCheckpoint[]).filter(
                  (item) => item.isCorrect,
                ).length,
                total: (item.resource as SelectionLessonResourse[]).length,
              };
              break;
            default:
              break;
