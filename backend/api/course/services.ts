...
import { CourseSchema } from '../../types/schema/courseSchemaTypes';
...
const courseServices = {
  ...
  getCourseById: async (req: Request): Promise<TServiceResponseType<TGetCourseByIdResponse>> => {
    ...
    if (user) {
      const concurrentPromise = lessons.map((lesson) =>
        UserLessonModel.findOne({ lessonId: lesson._id, courseId, userId: (req.user as TUserRole).id })
          .select('status')
          .then((item) => ({
            _id: lesson?._id,
    ...
    return {
      data: {
        ...
        isCurrentUserJoined: course.participantsId.some(
          (item) => item.userId === (req.user as TUserRole)?.id,
        ),
  ...
  updateCourseById: async (req: Request) => {
    const courseId = (req.params as TCourseById).courseId;
    const reqBody = req.body as CourseSchema;
