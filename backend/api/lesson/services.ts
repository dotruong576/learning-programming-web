...
import { LessonResource, VideoLessonResourse } from '../../types/schema/lessonSchemaTypes';

const lessonServices = {
  createLesson: async (req: Request) => {
    const reqBody = req.body as TCreateLessonPayload<LessonResource>;
    ...
    const createdLesson = await LessonModel.create({
      ...
      duration:
        (req.query.type as ELessonType) === ELessonType.Video
          ? parseInt((reqBody.resource as VideoLessonResourse).duration)
